...
export const KakaoMap = () => {
  ...
  const handleStoreInfo = async (id) => {
    try {
      const response = await axios.post(API_URLS.GET_STORE_INFO, {id})
            ...
            if (
              ...
              return (
                <SideBarContainer>
                  ...
                  <Container>
                    <h2 style={{fontWeight: '700'}}>#{marker.place} 길 찾기</h2>
                    <iframe
                      src={`https://map.kakao.com/?sName=강남역&eName=${marker.place}`}
                      title='Kakao Map'
                      frameborder='0'
                      allowfullscreen
                  ...
                  <Container>
                    <Row>
                      <h2 style={{fontWeight: '700'}}>
                        #{marker.place} 경로 미리 보기
                      </h2>
                      <p style={{color: 'grey'}}>
                        아래 지도 상에서 #{marker.place}의 위치를 자세히 확인해
                        보실 수 있습니다.
                      </p>
                    </Row>
                    <Row style={{height: '400px', backgroundColor: 'gray'}}>
                      <Map center={marker.position} draggable={false} level={1}>
                        <MapMarker
                          key={index}
                          position={marker.position}
                          image={{
